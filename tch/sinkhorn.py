...
def WasserstainLossLog(loga, logb, C, ϵ=1.0, max_iter=10):
    """
...
def approxSinkhornLoss(y, logit, C, ϵ, max_iter, **kwargs):
    ...
    #t3 = WasserstainLossLog(loga, loga, M3, ε, max_iter)
    return 2*t1 - t2 #- t3

def getSinkhornLoss(C, ε=1.0, max_iter=5):
    def SinkhornLoss(Y, logits, **kwargs):
        ...
        kwargs:
            ...
            k (int)
        """
        batch_size, loss = min(Y.shape[0], 200), 0.0
        indices, k = kwargs.get("indices", [None] * batch_size), kwargs.get("k", None)
...
def CELoss(Y, logits):
    """
    Args:
        Y (batch_size, m): ground truth.
        logits (batch_size, m): model output logits.
    """
    logPr = torch.log_softmax(logits, dim=1)
    return -torch.mean(torch.sum(Y * logPr, dim=1))

def L2Loss(Y, logits):
    return F.mse_loss(logits, Y)

def LogisticLoss(Y, logits):
    return torch.mean((1-Y)*logits - F.logsigmoid(logits))    